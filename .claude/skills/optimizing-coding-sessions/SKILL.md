...
description: >-
  ...
  Claude Code公式ベストプラクティスに基づきセッション最適化をガイドする。

  主要機能：
  - プロンプト設計パターン（検証手段、スコープ明確化、Before/After例）
  - コンテキスト管理（/clear、/compact、サブエージェント活用）
  - CLAUDE.md最適化（含めるべき/含めるべきでない内容の診断）
  - セッション運用・スケーリング（並列セッション、ファンアウト）

  トリガー: 「ベストプラクティス」「効率的な使い方」「コンテキスト管理」
  「プロンプト改善」「CLAUDE.md最適化」

  使用しない場面: コード実装・バグ修正・機能開発
---
