...
description: >-
  コンテキスト枯渇・指示の無視・繰り返し修正失敗などの問題発生時に、
  Claude Code公式ベストプラクティスに基づき4領域をガイドする：
  (1)プロンプト設計パターン (2)コンテキスト管理（/clear、/compact、サブエージェント）
  (3)CLAUDE.md最適化 (4)セッション運用・スケーリング。
  「ベストプラクティス」「効率的な使い方」「コンテキスト管理」「プロンプト改善」
  「CLAUDE.md最適化」で起動。
  コード実装・バグ修正・機能開発には使用しない。
---
...
課題に応じて参照ファイルを読み込み、以下を出力する：
- **該当するアンチパターン名**と現在の症状の対応付け
- **具体的な改善アクション**（コマンド、設定変更、プロンプト書き換え等）
- **Before/After例**（プロンプト改善の場合は必須）

### ステップ3: 実践的な改善提案

ユーザーの現在のワークフローをヒアリングし：
- 現在のCLAUDE.mdを読み込んで具体的な削除・移行候補を提示（該当する場合）
- 問題のプロンプトを受け取りBefore/After形式で書き換え例を提示（該当する場合）
- `/clear` + 改善プロンプトでの再実行手順を提示（該当する場合）
